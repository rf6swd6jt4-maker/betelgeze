...
- Device inventory orders installations by their latest Auth session creation time, with an explicit stable tie-breaker, independently of last-seen/refresh activity. Known installations remain deduplicated. Security initially renders two cards and an accessible See more/See less control for the rest. An older current device can appear in the expanded group; only its own notification control is editable.
...
- Explicit off and logout serialize revocation with registration/device ownership. Logout stays on the originating platform host so the route receives its host-only device cookie and original Origin header. It revokes this installation even if its HTTP login expired, then clears the device cookie and local auth. A revocation failure returns a retryable error instead of reporting a successful logout. Mutation routes require the same origin. Already accepted provider deliveries cannot be recalled; this is a boundary shared by unsubscribe, account switching and logout.
