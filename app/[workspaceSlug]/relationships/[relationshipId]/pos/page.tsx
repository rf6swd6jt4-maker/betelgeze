...
export default async function RelationshipDetailPage({ params }: PageProps) {
    ...
    if (native.data?.length || relationship.source_metadata.service_instances === true) {
        ...
        return <main className="min-h-screen bg-neutral-950 text-white"><WorkspaceTopBar userId={user.id} workspace={workspace} workspaceAccess={access} currentProduct="client-work" /><div className="mx-auto grid max-w-[92rem] gap-5 lg:grid-cols-[minmax(0,1fr)_auto]"><RelationshipServicePos key={`${user.id}:${relationshipId}`} workspaceSlug={workspace.slug} relationshipId={relationshipId} userId={user.id} initial={{...result.data,items:result.data.items.slice(0,30)}} relationship={{name:relationship.primary_person_name,company:relationship.business_name,email:relationship.primary_email,phone:relationship.primary_phone??relationship.whatsapp_phone,updatedAt:relationship.updated_at,managerId:relationship.fulfilment_manager_user_id}} /><ClientContextPanel access={access} workspaceSlug={workspace.slug} relationship={relationship} /></div></main>
    }
