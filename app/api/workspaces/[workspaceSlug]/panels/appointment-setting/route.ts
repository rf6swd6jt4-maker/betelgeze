...
import { loadNativeAppointment } from "@/lib/workspace-native-appointment"
...
export async function GET(request: Request, { params }: { params: Promise<{ workspaceSlug: string }> }) {
    ...
    if (id && !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id)) return NextResponse.json({ error: "Invalid relationship." }, { status: 400, headers: { "Cache-Control": "private, no-store" } })
    try {
        const snapshot = await loadNativeAppointment(workspaceSlug, id)
