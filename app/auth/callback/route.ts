...
export async function GET(request: NextRequest) {
    const url = request.nextUrl; const code = url.searchParams.get("code"); const requestedNext = url.searchParams.get("next") || "/dashboard"; const suiteNext = /^https:\/\/(dashboard|onboarding|leadgen)\.betelgeze\.com(?:\/|$)/.test(requestedNext); const next = suiteNext ? requestedNext : requestedNext.startsWith("/") && !requestedNext.startsWith("//") ? requestedNext : "/dashboard"
