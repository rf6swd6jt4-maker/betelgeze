...
export default async function ClientPortalSessionPage({ params }: PageProps) {
    ...
    const { workspace, relationship, theme, metaAdsReporting, hasGoogleAds, overview } = resolved
    ...
    return <OnboardingThemeProvider theme={theme}>
        <ClientPortalShell token={token} workspaceName={publicBranding.displayName} logoSrc={logoSrc} primaryPersonName={relationship.primary_person_name} overview={overview} metaAdsReporting={metaAdsReporting} hasGoogleAds={hasGoogleAds} privacyPolicyUrl={publicBranding.privacyPolicyUrl} termsOfServiceUrl={publicBranding.termsOfServiceUrl} />
