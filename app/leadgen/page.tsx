...
export default async function LeadgenIndexPage() {
  ...
  if (workspaces.length === 1) redirect(`https://leadgen.betelgeze.com/${workspaces[0].slug}`)
  ...
  return <main className="min-h-screen bg-neutral-950 px-6 py-16 text-white">
    <section className="mx-auto max-w-2xl rounded-2xl border border-neutral-800 bg-neutral-900 p-7 sm:p-8">
      ...
      {workspaces.length ? <div className="mt-7 grid gap-2">
        {workspaces.map((workspace) => <a key={workspace.slug} href={`https://leadgen.betelgeze.com/${workspace.slug}`} className="rounded-xl border border-neutral-800 bg-neutral-950 px-4 py-3 text-sm text-neutral-100 hover:border-neutral-600">{workspace.name}</a>)}
