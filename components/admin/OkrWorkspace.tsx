...
type Person = { user_id: string; role: string; name: string; avatarSrc?: string | null }
...
function FormAssignment({ name, defaultValue, people, ariaLabel, title }: { name: string; defaultValue: string; people: Person[]; ariaLabel: string; title: string }) {
    const [value, setValue] = useState(defaultValue)
    return <AssignmentSelector name={name} value={value} onChange={setValue} people={people.map((person) => ({ id: person.user_id, name: person.name, avatarSrc: person.avatarSrc, description: person.role }))} required appearance="input" ariaLabel={ariaLabel} title={title} />
}
...
function ObjectiveDetails({ workspaceSlug, okr, ownerOptions, today, pending, run, runWithoutForm, onAddResult }: { workspaceSlug: string; okr: WorkspaceOkr; ownerOptions: Person[]; today: string; pending: boolean; run: (event: FormEvent<HTMLFormElement>, action: RunAction, after?: AfterAction) => void; runWithoutForm: (action: () => Promise<void>, after?: AfterAction) => void; onAddResult: () => void }) {
    ...
    const [ownerId, setOwnerId] = useState(okr.owner_user_id)
    ...
    return <div>
        <form key={okr.updated_at} onSubmit={(event) => run(event, submitAction)} onChange={() => setDirty(true)} onReset={() => { setDirty(false); setOwnerId(okr.owner_user_id) }} className="p-4 sm:p-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="text-sm text-neutral-400"><p>Owner</p>{editable ? <span className="mt-1.5 block"><AssignmentSelector name="owner_user_id" value={ownerId} onChange={(value) => { setOwnerId(value); setDirty(true) }} people={ownerOptions.map((person) => ({ id: person.user_id, name: person.name, avatarSrc: person.avatarSrc, description: person.role }))} required appearance="input" ariaLabel="Objective owner" title="Assign Objective owner" /></span> : <span className="mt-1.5 flex min-h-10 items-center rounded-md border border-neutral-900 bg-black/40 px-3 py-2"><Assignee userId={okr.owner_user_id} name={ownerOptions.find((person) => person.user_id === okr.owner_user_id)?.name ?? "Admin"} avatarSrc={ownerOptions.find((person) => person.user_id === okr.owner_user_id)?.avatarSrc} /></span>}</div>
