...
import { diagnoseGoogleAdsOnboarding, saveWhatsAppConfirmationTemplate, saveWhatsAppOnboardingTemplate } from "@/app/[workspaceSlug]/settings/actions"
    ...
    return <section className={`${showHeader ? "mt-8 " : ""}min-w-0 max-w-full`}>
        ...
        {selected && connection ? <Modal title={`Connect ${titles[selected]}`} description="The current connection remains active until the replacement passes every required check." error={error} onClose={() => { if (!pending) setSelected(null) }}>
            <div className="space-y-5 p-4 sm:p-5">
                ...
                : selected === "meta_whatsapp" ? <div className="rounded-xl border border-neutral-800 p-4"><h3 className="font-medium text-white">Connect with Meta</h3><p className="mt-1 text-sm leading-6 text-neutral-500">Choose the agency&apos;s Meta business and WhatsApp number. Betelgeze verifies the channel-confirmation and onboarding-link templates separately.</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="block text-sm text-neutral-300">Confirmation template<input value={templateName} onChange={(event) => setTemplateName(event.target.value)} className={inputClass} /></label><label className="block text-sm text-neutral-300">Language<input value={templateLanguage} onChange={(event) => setTemplateLanguage(event.target.value)} className={inputClass} /></label></div>{connection.enabled && connection.mode === "connected" ? <><p className="mt-3 text-xs leading-5 text-neutral-400">The confirmation template remains for Retention and must use a CONFIRM reply.</p><button type="button" disabled={pending || !templateName.trim() || !templateLanguage.trim()} onClick={() => run(() => saveWhatsAppConfirmationTemplate(workspaceSlug, templateName, templateLanguage), true)} className="mt-3 h-10 w-full rounded-lg bg-white px-4 text-sm font-medium text-black disabled:opacity-50">{pending ? "Verifying…" : "Save confirmation template"}</button><div className="mt-5 border-t border-neutral-800 pt-5"><p className="text-sm font-medium text-white">Onboarding-link Utility template</p><p className="mt-1 text-xs leading-5 text-neutral-400">Use an approved Utility template with exactly one body variable for the secure onboarding URL. It sends without waiting for a reply.</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="block text-sm text-neutral-300">Template<input value={onboardingTemplateName} onChange={(event) => setOnboardingTemplateName(event.target.value)} placeholder="onboarding_link" className={inputClass} /></label><label className="block text-sm text-neutral-300">Language<input value={onboardingTemplateLanguage} onChange={(event) => setOnboardingTemplateLanguage(event.target.value)} className={inputClass} /></label></div><button type="button" disabled={pending || !onboardingTemplateName.trim() || !onboardingTemplateLanguage.trim()} onClick={() => run(() => saveWhatsAppOnboardingTemplate(workspaceSlug, onboardingTemplateName, onboardingTemplateLanguage), true)} className="mt-3 h-10 rounded-lg bg-white px-4 text-sm font-medium text-black disabled:opacity-50">{pending ? "Verifying…" : "Save onboarding-link template"}</button></div></> : null}<button type="button" disabled={pending || !templateName.trim()} onClick={startWhatsApp} className="mt-4 h-10 w-full rounded-lg bg-white px-4 text-sm font-medium text-black disabled:opacity-50">Continue with Meta</button></div>
