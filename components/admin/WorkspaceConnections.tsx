...
import { diagnoseGoogleAdsOnboarding, saveWhatsAppTemplates } from "@/app/[workspaceSlug]/settings/actions"
...
function CapabilityList({ connection }: { connection: WorkspaceConnection }) {
    const labels: Record<string, string> = connection.provider === "stripe"
        ...
        : connection.provider === "meta_whatsapp"
            ? { phone_access: "Phone number accessible", outbound_messages: "Outbound messaging allowed", webhook_subscribed: "Incoming events routed", consent_template_approved: "Confirmation approved", reconfirmation_template_approved: "Reconfirmation approved", onboarding_template_approved: "Onboarding link approved", client_portal_template_approved: "Client portal link approved" }
...
function TemplateFields({ title, description, name, language, setName, setLanguage }: { title: string; description: string; name: string; language: string; setName: (value: string) => void; setLanguage: (value: string) => void }) {
    return <fieldset className="rounded-lg border border-neutral-800 p-3"><legend className="px-1 text-sm font-medium text-white">{title}</legend><p className="text-xs leading-5 text-neutral-500">{description}</p><div className="mt-2 grid gap-3 sm:grid-cols-[minmax(0,1fr)_8rem]"><label className="block text-xs text-neutral-400">Template name<input value={name} onChange={(event) => setName(event.target.value)} className={inputClass} /></label><label className="block text-xs text-neutral-400">Language<input value={language} onChange={(event) => setLanguage(event.target.value)} className={inputClass} /></label></div></fieldset>
}
...
export function WorkspaceConnections({ workspaceSlug, connections, verifyAction, manualAction, completeWhatsAppAction, selectMetaAdsBusinessAction, verifyPendingAction, discardPendingAction, rollbackAction, disconnectAction, canManage, canManageHighLevel = canManage, metaAppId, metaEmbeddedSignupConfigId, showHeader = true }: Props) {
    ...
    const [onboardingTemplateName, setOnboardingTemplateName] = useState("")
    const [onboardingTemplateLanguage, setOnboardingTemplateLanguage] = useState("en_US")
    const [reconfirmationTemplateName, setReconfirmationTemplateName] = useState("scaylup_service_updates_preference")
    const [reconfirmationTemplateLanguage, setReconfirmationTemplateLanguage] = useState("en")
    const [clientPortalTemplateName, setClientPortalTemplateName] = useState("scaylup_client_portal_access")
    const [clientPortalTemplateLanguage, setClientPortalTemplateLanguage] = useState("en")
    ...
    return <section className={`${showHeader ? "mt-8 " : ""}min-w-0 max-w-full`}>
        ...
        {selected && connection ? <Modal title={`Connect ${titles[selected]}`} description="The current connection remains active until the replacement passes every required check." error={error} onClose={() => { if (!pending) setSelected(null) }}>
            <div className="space-y-5 p-4 sm:p-5">
                ...
                : selected === "meta_whatsapp" ? <div className="rounded-xl border border-neutral-800 p-4"><h3 className="font-medium text-white">WhatsApp connection</h3><p className="mt-1 text-sm leading-6 text-neutral-500">The connected business number and every required Utility template are verified with Meta before anything is saved.</p>{connection.enabled ? <div className="mt-4 rounded-lg border border-neutral-800 bg-neutral-900 px-3 py-3"><p className="text-xs uppercase tracking-wide text-neutral-500">Connected number</p><p className="mt-1 text-sm font-medium text-white">{String(connection.config_hint.display_phone_number || "Number unavailable — verify the connection")}</p></div> : null}<div className="mt-4 grid gap-4"><TemplateFields title="Confirmation" description="Initial consent request. Must ask the client to CONFIRM." name={templateName} language={templateLanguage} setName={setTemplateName} setLanguage={setTemplateLanguage} /><TemplateFields title="Reconfirmation" description="Reopens an expired WhatsApp response window. Must ask the client to CONFIRM." name={reconfirmationTemplateName} language={reconfirmationTemplateLanguage} setName={setReconfirmationTemplateName} setLanguage={setReconfirmationTemplateLanguage} /><TemplateFields title="Onboarding link" description="Sent at the end of POS. Requires exactly one secure-link variable." name={onboardingTemplateName} language={onboardingTemplateLanguage} setName={setOnboardingTemplateName} setLanguage={setOnboardingTemplateLanguage} /><TemplateFields title="Client portal link" description="Requires exactly one secure-link variable." name={clientPortalTemplateName} language={clientPortalTemplateLanguage} setName={setClientPortalTemplateName} setLanguage={setClientPortalTemplateLanguage} /></div>{connection.enabled && connection.mode === "connected" ? <button type="button" disabled={pending || [templateName, templateLanguage, reconfirmationTemplateName, reconfirmationTemplateLanguage, onboardingTemplateName, onboardingTemplateLanguage, clientPortalTemplateName, clientPortalTemplateLanguage].some((value) => !value.trim())} onClick={() => run(() => saveWhatsAppTemplates(workspaceSlug, { confirmation: { name: templateName, language: templateLanguage }, reconfirmation: { name: reconfirmationTemplateName, language: reconfirmationTemplateLanguage }, onboarding: { name: onboardingTemplateName, language: onboardingTemplateLanguage }, clientPortal: { name: clientPortalTemplateName, language: clientPortalTemplateLanguage } }), true)} className="mt-5 h-10 w-full rounded-lg bg-white px-4 text-sm font-medium text-black disabled:opacity-50">{pending ? "Verifying all templates…" : "Save templates"}</button> : <button type="button" disabled={pending || !templateName.trim()} onClick={startWhatsApp} className="mt-5 h-10 w-full rounded-lg bg-white px-4 text-sm font-medium text-black disabled:opacity-50">Continue with Meta</button>}</div>
                ...
                : selected === "windsor" ? <form onSubmit={submitManual} data-workspace-mutation-scope="local" className="space-y-4 rounded-xl border border-neutral-800 p-4">
                    <div><h3 className="font-medium text-white">Connect Windsor.ai</h3><p className="mt-2 text-sm leading-6 text-neutral-400">Betelgeze will use Windsor&apos;s external authorization links during client onboarding and its reporting API for the client portal. The direct Betelgeze Meta App remains separate.</p></div>
