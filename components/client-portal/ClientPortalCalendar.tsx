...
export function ClientPortalCalendar({token,active}:{token:string;active:boolean}) {
 ...
 useEffect(()=>{mounted.current=true;const requestGeneration=generation;return()=>{mounted.current=false;requestGeneration.current++;controller.current?.abort();controller.current=null}},[])
