...
import type { GhlMetrics, GhlSummary } from "@/lib/client-portal/ghl-types"
...
const sampleMetrics: GhlMetrics = { contacts: 164, opportunities: 48, open: 28, won: 14, lost: 6 }
...
export function ClientPortalGhl({ token, active }: { token: string; active: boolean }) {
    ...
    async function mutate(action: "connect" | "refresh" | "disconnect") {
        ...
        try {
            ...
            setSaved(value); setSample(false); setEditing(false); setConfirmDisconnect(false)
    ...
    const metrics = sample ? sampleMetrics : saved?.metrics
    const connected = saved?.connected === true
    const beginEditing = () => { setLocationId(saved?.locationId ?? locationId); setEditing(true); setSample(false); setConfirmDisconnect(false) }
    ...
    return <PortalSection id="ghl-connection" title="GHL" description="Contacts & opportunities" icon="connection">
        <div className="mt-4 min-w-0" aria-busy={Boolean(pending)}>
            <Status surface="light" tone={sample ? "yellow" : connected ? "green" : "grey"} wrap label={sample ? "Sample data · not connected" : saved?.busy ? "Update in progress…" : connected ? "Connected" : reading || !saved ? "Checking connection…" : "Not connected"} />
            {sample ? <p className="mt-2 text-sm text-[var(--onboarding-muted,#475569)]">Preview for this TEST portal. These figures are examples.</p> : connected ? <p className="mt-2 break-words text-sm font-medium">{saved.locationName}</p> : <p className="mt-3 text-sm leading-6 text-[var(--onboarding-muted,#475569)]">Connect your GHL sub-account to see your contacts and opportunity results here.</p>}
            {metrics ? <>
                <QuickStats surface="light" ariaLabel={sample ? "Sample opportunity metrics" : "GHL opportunity metrics"} items={[
                ...
                {!sample && saved?.refreshedAt ? <p className="mt-2 text-xs leading-5 text-[var(--onboarding-muted,#475569)]">Updated {new Date(saved.refreshedAt).toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" })}. Refresh to update.</p> : null}
            </> : null}
            {error || (!sample && saved?.error) ? <div role="alert" className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm leading-5 text-red-800"><p>{error || saved?.error}</p><button type="button" className={`${secondary} mt-1`} disabled={Boolean(pending) || reading} onClick={() => void load()}>Reload status</button></div> : null}
            {editing ? <form onSubmit={submit} className="mt-4 space-y-4">
                ...
                {sample ? <button type="button" className={secondary} onClick={() => setSample(false)}>Back to connection</button> : connected ? <><button type="button" className={portalPrimaryButton} disabled={Boolean(pending)} onClick={() => void mutate("refresh")}>{pending === "refresh" ? "Refreshing…" : "Refresh metrics"}</button><button type="button" className={secondary} disabled={Boolean(pending)} onClick={beginEditing}>Manage connection</button></> : <><button type="button" className={portalPrimaryButton} disabled={Boolean(pending) || !saved} onClick={beginEditing}>Connect GHL</button><button type="button" className={secondary} onClick={() => setSample(true)}>Preview metrics</button></>}
            </div>}
