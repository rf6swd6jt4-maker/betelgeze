...
export function ClientPortalActions({ active = true, workspaceSlug, relationshipId }: { active?: boolean; workspaceSlug: string; relationshipId: string }) {
    ...
    useLayoutEffect(() => {
        ...
        return () => { requests.id++; setOpen(false); setEditingProgress(null) }
    }, [active])
    const close = () => { requestRef.current.id++; setOpen(false); setEditingProgress(null) }
    ...
    const load = useCallback(async () => {
        const request = ++requestRef.current.id
        setLoading(true); setError(null)
        try {
            const next = await portalRequest(endpoint)
            if (request === requestRef.current.id) setOverview(next)
        } catch (reason) {
            if (request === requestRef.current.id) setError(reason instanceof Error ? reason.message : "The client portal actions could not be loaded.")
        } finally { if (request === requestRef.current.id) setLoading(false) }
    }, [endpoint])
    const toggle = () => {
    ...
    const update = async (input: { kind: "action" | "progress"; id: string; status: string; updatedAt: string }) => {
        ...
        const request = ++requestRef.current.id
        setLoading(false); setPending(input.id); setError(null)
        try {
            const next = await portalRequest(endpoint, { method: "PATCH", body: JSON.stringify(input) })
            if (request === requestRef.current.id) { setOverview(next); setEditingProgress(null) }
        } catch (reason) {
            if (request === requestRef.current.id) setError(reason instanceof Error ? reason.message : "The client portal action could not be updated.")
    ...
    const add = async (event: FormEvent) => {
        ...
        pendingRef.current = true
        const request = ++requestRef.current.id
        setLoading(false); setPending("new"); setError(null)
        try {
            const overview = await portalRequest(endpoint, { method: "POST", body: JSON.stringify({ title: next }) })
            if (request === requestRef.current.id) setOverview(overview)
            setTitle(current => current.trim() === next ? "" : current)
        } catch (reason) {
            if (request === requestRef.current.id) setError(reason instanceof Error ? reason.message : "The client portal action could not be added.")
        } finally { pendingRef.current = false; setPending(null) }
    }
    ...
    const addAction = !progress ? <form onSubmit={add} className="flex min-w-0 gap-2"><label className="sr-only" htmlFor={`portal-action-${relationshipId}`}>New required action</label><input id={`portal-action-${relationshipId}`} value={title} disabled={pending !== null} maxLength={240} onChange={(event) => setTitle(event.target.value)} placeholder="Add required action" className="h-10 min-w-0 flex-1 rounded-lg border border-neutral-700 bg-black px-3 text-sm outline-none placeholder:text-neutral-600 focus:border-neutral-500" /><button type="submit" disabled={!title.trim() || pending !== null} className="h-10 shrink-0 rounded-lg bg-white px-3 text-sm font-semibold text-black disabled:opacity-40">Add</button></form> : undefined

    return <>
        <button type="button" aria-label="Client portal actions" aria-expanded={active && open} disabled={pending !== null} onClick={toggle} className="relative inline-flex h-10 w-10 shrink-0 items-center justify-center text-neutral-400 outline-none transition hover:text-white focus-visible:ring-2 focus-visible:ring-neutral-600">
            <ActionsIcon />
        ...
        {active && open ? <CenteredDialog title="Client portal" short busy={pending !== null} onClose={close} footer={addAction}>
            ...
            {progress ? <div className="mt-4">
                <button type="button" disabled={pending !== null} onClick={() => setEditingProgress(null)} className="mb-2 inline-flex min-h-9 items-center text-sm text-neutral-400 hover:text-white">← Back</button>
                ...
                <div className="grid gap-1">{statusOptions.map((status) => <button key={status} type="button" disabled={pending !== null} onClick={() => void update({ kind: "progress", id: progress.id, status, updatedAt: progress.updatedAt })} className={`flex min-h-11 items-center justify-between rounded-lg px-3 text-left text-sm hover:bg-neutral-900 disabled:opacity-50 ${progress.status === status ? "bg-neutral-900 text-white" : "text-neutral-300"}`}><span>{progressLabels[status]}</span>{progress.status === status ? <span aria-hidden="true">✓</span> : null}</button>)}</div>
