...
function realtimeMessage(value: unknown): CommunicationMessage | null {
    ...
    return {
        ...
        uploadedAssetId: direction === "inbound" && row.sender_kind === "client" ? resourceUploadAssetId(row.raw_payload) : null,
    ...
    async function toggleCheckbox(message: CommunicationMessage, line: number, checked: boolean, expectedBody: string) {
        ...
        try {
            await updates.mutateMessage(message.id, { ...message, body }, async () => {
                savedBody = await requestChatCheckbox(`/api/workspaces/${bootstrap.workspaceSlug}/communications/checklist`, { relationshipId: message.relationshipId, messageId: message.id, line, checked, expectedBody })
