...
export function GoogleAdsAccountPicker({ state }: { state: string }) {
    ...
    const [view, setView] = useState<View | null>(null), [selected, setSelected] = useState(""), [pending, setPending] = useState(false), [error, setError] = useState("")
    useEffect(() => {
    ...
    useEffect(() => {
        if (!done || !view?.origin) return
        window.opener?.postMessage({ type: "betelgeze-google-ads" }, view.origin)
        const close = window.setTimeout(() => window.close(), 500)
        return () => window.clearTimeout(close)
    }, [done, view?.origin])
    async function connect() {
        if (!selected || pending) return
        setPending(true); setError("")
        try { const result = await adsFetch(api, { customerId: selected, consented: true }); setView(current => current ? { ...current, phase: "done", status: result.status, origin: result.origin } : null) }
    ...
    return <main className="min-h-dvh bg-[#F8F7F4] px-4 py-10 text-[#0F172A]"><section className="mx-auto max-w-lg rounded-2xl border border-black/10 bg-white p-6 shadow-sm" aria-busy={pending}>
        ...
        {!view && !error ? <p className="mt-5 text-sm" role="status">Loading your accounts…</p> : done ? <div className="mt-5 space-y-4"><p className="text-sm leading-6">{view.status === "connected" ? "Your Google Ads account is connected. Return to your portal or onboarding to see its status." : "Your agency’s access request is ready. Return to your portal or onboarding for the remaining Google Ads approval steps."}</p><button className={adsPrimary} onClick={() => window.close()}>Close this window</button><p className="text-xs text-slate-600">If this window stays open, switch back to your original tab and reload the connection status.</p></div> : view?.phase === "ready" ? <form className="mt-5 space-y-5" onSubmit={e => { e.preventDefault(); void connect() }}>
            ...
            {view.choices.length ? <Selector surface="light" appearance="input" ariaLabel="Advertising account" value={selected} options={view.choices.map(choice => ({ value: choice.id, label: choice.name, description: formatGoogleAdsCustomerId(choice.id) }))} onChange={setSelected} disabled={pending} /> : <p className="text-sm">No active advertising accounts were available. Close this window and sign in with an account that has Google Ads access, or use your customer ID.</p>}
            ...
            {selected ? <p className="text-sm leading-6 text-slate-600">By selecting Approve and connect, you confirm you are authorised to grant {view.managerName} manager access to account {formatGoogleAdsCustomerId(selected)} for campaign management and reporting.</p> : null}
            <button className={adsPrimary} disabled={!selected || pending}>{pending ? "Connecting…" : "Approve and connect"}</button>
