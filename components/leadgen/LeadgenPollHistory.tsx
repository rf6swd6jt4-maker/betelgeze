...
export function LeadgenPollHistory({ workspace, userId, polls, olderHref, hasCursor }: {
    ...
    return <main className="min-h-screen bg-neutral-950 px-4 pb-5 text-white sm:px-6 sm:pb-6"><div className="mx-auto max-w-7xl">
        ...
        <PanelTabHeader title="Poll history" description="Read-only archive of saved Lead Gen polls." />
        <List ariaLabel="Lead generation poll history">
            {polls.map((poll) => {
                ...
                return <ListItem key={poll.id}>
                    <ListPrimaryRow><ListTitle href={`/${workspace.slug}/leadgen/poll/${poll.id}`}>Poll {shortId(poll.id)}</ListTitle><Status tone={tone} label={poll.status === "queued" || poll.status === "running" ? `${poll.status} · retired` : poll.status} /></ListPrimaryRow>
