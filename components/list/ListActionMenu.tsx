...
export function ListActionMenu({ actions, label = "Open item actions" }: { actions: Array<Partial<ListAction> | null | undefined | false>; label?: string }) {
    ...
    return <div ref={menuRef} className="relative z-30 shrink-0">
        <button type="button" onClick={toggle} aria-label={label} aria-expanded={open} aria-haspopup="menu" className="inline-flex h-8 w-8 items-center justify-center text-white hover:text-neutral-300 focus:outline-none focus:ring-2 focus:ring-white/30">
            <span aria-hidden="true" className="flex items-center gap-1">
                <span className="h-1.5 w-1.5 rounded-full bg-current" />
                <span className="h-1.5 w-1.5 rounded-full bg-current" />
                <span className="h-1.5 w-1.5 rounded-full bg-current" />
        ...
        {open && <div role="menu" className="absolute right-0 z-50 mt-2 w-[calc(100vw-2rem)] max-w-52 overflow-hidden rounded-lg border border-neutral-800 bg-neutral-950 shadow-2xl">
