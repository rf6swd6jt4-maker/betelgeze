...
export function VisualBuilderCanvas({
    ...
    async function uploadVideo(block: VideoBlock | CrmSetupBlock, file: File) {
        setUploadingId(block.id); setUploadProgress(0); setUploadError(null)
        try {
            ...
            await uploadBuilderVideo({ uploadUrl: prepared.uploadUrl, file, onProgress: setUploadProgress })
            updateDraftRevisionId(prepared.draftRevisionId)
            const upload = { ...prepared.storedVideo, resolvedUrl: prepared.previewUrl }
            replaceBlock(block.kind === "video" ? { ...block, legacyEmbedUrl: null, upload } : { ...block, video: upload })
