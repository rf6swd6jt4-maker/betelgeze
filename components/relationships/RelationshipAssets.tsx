...
import {AttachmentPreview,AttachmentsBlock,CenteredDialog,AutoGrowTextarea} from '@/components/ui'
...
export function RelationshipAssets({slug,relationshipId,userId}:{slug:string;relationshipId:string;userId:string}){
 ...
 const section=useRef<HTMLDivElement>(null),busyRef=useRef(false),dirtyRef=useRef(false)
 ...
 return <div ref={section}>{error&&!open?<p role="alert" className="mb-3 text-sm text-red-300">{error} <button className="underline" onClick={()=>void read().then(()=>setError('')).catch(e=>setError(e.message))}>Retry</button></p>:null}<AttachmentsBlock actions={<>{loaded?<button className={button} onClick={()=>void read().then(()=>setError('')).catch(e=>setError(e.message))}>Refresh</button>:null}{canEdit?<button className={button} onClick={add}>Attach file</button>:null}</>} description="AI reads these documents when generating new work. Attachment descriptions correct their documents; the relationship description overrides both. Existing work stays unchanged." empty={visible&&!loaded?'Loading attachments…':loaded?'No attachments yet.':'Attachments load when this section is visible.'}>
 {loaded&&items.length?items.map(a=><AttachmentPreview key={a.asset_id} href={`${endpoint}/${a.asset_id}`} title={a.title} subtitle={`${a.description||'Document ready for AI context'} · ${Math.ceil(a.file_size/1024)} KB`} contentType={a.content_type} actions={<div className="flex flex-wrap gap-1.5">{canEdit?<button className={button} onClick={()=>{setEditing(a);setDescription(a.description);setError('');setOpen(true)}}>Edit</button>:null}<a className={button+' inline-flex items-center'} href={`${endpoint}/${a.asset_id}`}>Download</a></div>} />):null}
 </AttachmentsBlock>
