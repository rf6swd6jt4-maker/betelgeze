...
export function ContactThumbnail({ method }: { method: Method }) {
    return method === "meta_whatsapp" || method === "twilio_sms" || method === "phone" ? <span className="flex scale-[1.7]"><CommunicationMethodMark method={method} /></span> : <svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-7 w-7">{method === "email" ? <><rect x="3" y="5" width="18" height="14" rx="2"/><path d="m3 6 9 7 9-7"/></> : <><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8m-4-4v4"/></>}</svg>
}
export function RelationshipContactCards({ workspaceSlug, relationshipId, userId, revision, canAdd }: {
    ...
    return <section ref={host} className="mt-6" aria-label="Relationship contacts"><h2 className="mb-3 text-base font-semibold">Contact</h2>
        <AttachmentCards label="Contact methods">{methods.map(method => { const choice = choices.find(choice => choice.provider === method); const broken = choice?.state === "broken" || method === "portal" && !data?.portal?.active; return <AttachmentCard key={method} title={contactMethodName(method)} thumbnail={<ContactThumbnail method={method} />} inactive={choice?.state === "inactive"} broken={broken} subtitle={method === "email" ? background.draft.primaryEmail : method === "phone" ? background.draft.primaryPhone : method === "portal" ? broken ? "Unavailable" : "Client access" : choice?.state === "active" ? "Confirmed" : broken ? "Needs attention" : "Not confirmed"} onClick={() => open(method)} /> })}{canAdd ? <AddAttachmentCard label="Add contact" onClick={() => open("add")} /> : null}</AttachmentCards>
