...
export function RelationshipEngagement({ workspaceSlug, relationshipId, userId }: { workspaceSlug: string; relationshipId: string; userId: string }) {
    ...
    useEffect(() => {
        if (!active || !visible || (retained.get(key) && Date.now() - retained.get(key)!.at < 60_000)) return
    ...
    return <section ref={sectionRef} aria-label="Client engagement" className="mt-6 border-t border-neutral-800 pt-5">
