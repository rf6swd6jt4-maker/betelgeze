...
export function RelationshipServicePos(props: Props) {
    ...
    function toggleDelivery(choice: NonNullable<ServicePosPage["contacts"]>[number]) {
        const selected = input.delivery?.some((item) => item.provider === choice.provider)
        const canUseUtilityTemplate = choice.provider === "meta_whatsapp" && choice.enabled && choice.state !== "broken"
        if (!selected && canUseUtilityTemplate && choice.state !== "active" && !window.confirm("Warning: the client has not opted in to this WhatsApp channel. Finishing this sale will send the approved Utility onboarding-link template without waiting for a client reply. Continue?")) return
        setInput(current => ({ ...current, delivery: selected ? current.delivery?.filter(item => item.provider !== choice.provider) : [...(current.delivery ?? []), { provider: choice.provider, address: choice.address }] }))
    }
    return <div className="min-w-0 text-white">
            ...
            {step === 1 ? <>
                ...
                <h3 className="mb-2 font-semibold">Send their onboarding link</h3><p className="mb-4 text-sm leading-6 text-neutral-400">Choose the contact methods that will receive the onboarding link. An approved WhatsApp Utility template can be sent without a client reply.</p>
                <AttachmentCards label="Onboarding delivery methods">{(page.contacts ?? []).filter(choice => choice.added).map(choice => {
                    const canUseUtilityTemplate = choice.provider === "meta_whatsapp" && choice.enabled && choice.state !== "broken"
                    const available = choice.state === "active" ? choice.canSend || canUseUtilityTemplate : canUseUtilityTemplate
                    return <AttachmentCard key={choice.provider} title={contactMethodName(choice.provider)} thumbnail={<ContactThumbnail method={choice.provider} />} subtitle={choice.state === "active" ? choice.canSend ? choice.address : "Utility template" : choice.state === "broken" ? "Needs attention" : canUseUtilityTemplate ? "Utility template · no reply needed" : "Confirmation required"} selected={input.delivery?.some(selected => selected.provider === choice.provider)} inactive={!available} broken={choice.state === "broken"} disabled={locked || !available} onClick={() => toggleDelivery(choice)} />
                })}</AttachmentCards>
                {!(page.contacts ?? []).some(choice => choice.added && (choice.state === "active" && choice.canSend || choice.provider === "meta_whatsapp" && choice.enabled && choice.state !== "broken")) ? <p className="mt-3 text-sm text-neutral-400">Add a connected WhatsApp or Twilio contact before selling. WhatsApp can use the approved onboarding-link Utility template without a client reply.</p> : null}
