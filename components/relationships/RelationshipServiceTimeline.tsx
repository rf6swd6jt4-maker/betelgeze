...
export function RelationshipQueue({ endpoint, slug, relationshipId, userId, revision, publishedQueue, onGeneration }: {endpoint:string;slug:string;relationshipId:string;userId:string;revision:unknown;publishedQueue?:RelationshipQueuePage|null;onGeneration?:(instanceId:string)=>void}) {
    ...
    return <div ref={host} className="mt-6" aria-label="Relationship work queue"><h2 className="mb-3 text-base font-semibold">Queue</h2>
        ...
        {!data ? <p role="status" className="py-5 text-sm text-neutral-500">Loading work queue…</p> : <><List ariaLabel="Relationship work queue">{data.items.length ? data.items.map(item => {
        ...
        }) : <p className="px-4 py-5 text-sm text-neutral-500">No open work for this relationship.</p>}</List><Paging page={page} hasMore={data.hasMore} change={next=>{setPage(next);setData(null)}} /></>}
    </div>
}
