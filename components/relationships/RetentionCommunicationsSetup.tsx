...
export function RetentionCommunicationsSetup({ workspaceSlug, relationshipId, pending, canRequest, provider }: {
    workspaceSlug: string; relationshipId: string; pending: boolean; canRequest: boolean; provider: string
}) {
    ...
    return <section className="mb-5 space-y-3 border-b border-neutral-800 py-4" aria-label="Retention communications">
        <Status label={pending ? "Portal ready · Messaging not confirmed" : "Messaging confirmed"} tone={pending ? "yellow" : "green"} />
        <p className="text-sm text-neutral-400">The seller’s private portal handoff is in <Link href={`/${workspaceSlug}/communications?mode=team`} className="text-white underline">Comms → Team → BE</Link>. {pending ? "Client messages stay in the portal until their messaging channel is confirmed." : "The client can keep using the same portal link."}</p>
