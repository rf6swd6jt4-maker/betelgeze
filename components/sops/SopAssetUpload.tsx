...
export function SopAssetUpload({ workspaceSlug, workspaceId, userId, sopId }: { workspaceSlug: string; workspaceId: string; userId: string; sopId: string }) {
    ...
    return <>
        ...
        {open ? <CenteredDialog title="Add asset" busy={busy} onClose={() => setOpen(false)}><div className="space-y-3">
            ...
            <label className="block text-xs text-neutral-500">Guidance (optional)<AutoGrowTextarea value={notes} maxLength={2000} rows={2} disabled={busy || Boolean(pending)} onChange={e => updateGuidance(role, e.target.value)} className={`${sopInputClass} mt-1`} placeholder="Industry, when this applies, or which document this revises" /></label>
