...
import { AutoGrowTextarea, CenteredDialog } from "@/components/ui"
...
export function SopCatalogue({ workspaceSlug, workspaceId, userId, canAdd, items, next, paged, archived = false }: { workspaceSlug: string; workspaceId: string; userId: string; canAdd: boolean; items: Omit<SopRecord, "description">[]; next: string | null; paged: boolean; archived?: boolean }) {
    ...
    function closeCreate() {
        if (busy) return
        try { write(null); setOpen(false); setError("") } catch { setError("Could not clear the draft.") }
    }
    return <section className="mt-5">
        ...
        {canAdd && (open || Boolean(stored)) ? <CenteredDialog title="Add SOP" busy={busy} onClose={closeCreate}><form onSubmit={e => { e.preventDefault(); void create() }} className="space-y-4">
            <p className="text-xs leading-5 text-neutral-500">Name the procedure, then add its main document and supporting assets.</p>
            ...
            <div className="flex items-center justify-end gap-3"><button type="button" disabled={busy} className="min-h-10 px-2 text-sm text-neutral-400" onClick={closeCreate}>Cancel</button><button disabled={busy || !draft.title.trim()} className={sopButtonClass}>{busy ? "Creating…" : "Create SOP"}</button></div>
