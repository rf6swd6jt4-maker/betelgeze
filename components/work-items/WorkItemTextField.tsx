    ...
    return <DetailField label={label} icon="description" multiline stackOnMobile className="lg:col-span-2">
        <div>
            <textarea ref={draft.ref} aria-label={label} value={draft.value} onChange={event => draft.change(event.target.value)} onBlur={() => void draft.save()}
                ...
                className={`${label === "Instructions" ? "min-h-20" : "min-h-12"} w-full resize-none overflow-hidden bg-transparent py-0 text-sm leading-6 text-neutral-200 caret-neutral-300 outline-none placeholder:text-neutral-600 selection:bg-neutral-600 selection:text-white`} />
