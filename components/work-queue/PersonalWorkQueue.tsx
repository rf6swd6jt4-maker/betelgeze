...
export function PersonalWorkQueue({ data }: { data: PersonalQueueSnapshot }) {
    ...
    function openDispute(item: RankedQueueItem) { setReason("");setNote("");setDispute(item) }
    const tone = (item: RankedQueueItem) => item.state === "In progress" ? "yellow" as const : item.state === "Ready" ? "green" as const : "grey" as const
    return <>
        <PanelTabHeader title="Work Queue" description="Your ready work, ordered by value, timing and what it enables." actions={<button className={button} disabled={Boolean(busy)} onClick={()=>void refresh()}>Refresh</button>} />
        ...
        <List ariaLabel="Personal work queue">
            {featured ? <ListItem className="[content-visibility:visible]">
                <ListPrimaryRow><ListTitle href={href(featured.id)} className="flex-1">{featured.title}</ListTitle><Status label={featured.state === "In progress" ? "In progress" : "Up next"} tone={tone(featured)} /></ListPrimaryRow>
                ...
                <div className="space-y-3 border-t border-neutral-900 px-3.5 py-4 sm:px-4" data-queue-feature>
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-neutral-500">
                        ...
                        {featured.assessment ? <span>Estimated effort: {featured.assessment.effort_minutes} min</span> : null}
                        {featured.due_at ? <span>Due {new Date(featured.due_at).toLocaleString("en-IE",{dateStyle:"medium",timeStyle:"short",timeZone:"UTC"})} UTC</span> : null}
            ...
            {rows.map(item => {
                ...
                return <ListItem key={item.id}><MobileListActionSurface actions={actions} label={`Actions for ${item.title}`}><ListPrimaryRow><ListTitle href={href(item.id)} className="flex-1">{item.title}</ListTitle><Status label={item.state} tone={tone(item)} /></ListPrimaryRow><ListSecondaryRow><span className="hidden min-w-0 flex-1 truncate text-neutral-400 sm:inline">{item.relationship ? `${item.relationship} · ` : ""}{item.reason}</span>{item.assessment ? <span className="shrink-0 text-neutral-500">~{item.assessment.effort_minutes}m</span> : null}<ListTrailing><span className="font-mono text-neutral-500">{shortId(item.id)}</span><span className="text-neutral-500">{formatRelativeTime(item.updated_at)}</span><ListActionMenu className="hidden sm:block" actions={actions} /></ListTrailing></ListSecondaryRow></MobileListActionSurface></ListItem>
            })}
        ...
        {dispute ? <CenteredDialog title="Dispute work item" onClose={()=>setDispute(null)}><p className="mb-4 text-sm text-neutral-300">{dispute.title}</p><fieldset className="space-y-1"><legend className="mb-2 text-sm text-neutral-400">What is stopping you?</legend>{DISPUTE_REASONS.map(value=><label key={value} className="flex min-h-11 cursor-pointer items-center gap-3 rounded-lg px-2 text-sm hover:bg-neutral-900"><input type="radio" name="dispute-reason" value={value} checked={reason===value} onChange={()=>setReason(value)} />{value}</label>)}</fieldset><label className="mt-4 block text-sm text-neutral-400">{reason==="Other"?"Explanation":"Additional details (optional)"}<textarea className="mt-2 min-h-24 w-full resize-none rounded-lg border border-neutral-700 bg-neutral-900 p-3 text-white" value={note} onChange={e=>setNote(e.target.value)} maxLength={3000} required={reason==="Other"} /></label><p className="mt-3 text-xs text-neutral-500">Dispute submission is coming in a later release. Nothing entered here is sent or saved.</p></CenteredDialog> : null}
