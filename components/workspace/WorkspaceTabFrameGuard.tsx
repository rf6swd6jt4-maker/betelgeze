...
import { createWorkspaceFrameNavigator, WORKSPACE_FRAME_NAVIGATION_ATTRIBUTE, WORKSPACE_FRAME_NAVIGATION_EVENT } from "@/lib/workspace-frame-navigation"
...
export function WorkspaceTabFrameGuard() {
    ...
    useEffect(() => {
        ...
        function navigate(url: string, replace = false) {
            ...
            void navigator.navigate(workspaceTabFrameUrl(url, tabId!, window.location.origin), replace).catch(() => {
                // Failed draft persistence must stop navigation, never discard it.
                window.parent.postMessage({ source: WORKSPACE_TAB_MESSAGE_SOURCE, target: "host", tabId, type: "navigation-failed", url }, window.location.origin)
            })
        }
