...
Stored push is independent of HTTP token expiry. Device permission and explicit off remain authoritative. Logout must acknowledge server revocation before clearing the cookie. Provider-accepted messages can already be in transit at logout or account switching; they cannot be recalled. Notification navigation still goes through the application's usual authentication and conversation authorization.
