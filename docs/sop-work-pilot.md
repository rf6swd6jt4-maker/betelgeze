...
4. Add service switches the shared dialog to **Generating work…**. A small authenticated status read every 2.5 seconds reports durable milestones, bounded to eight minutes and paused in inactive/hidden tabs. Requests never overlap. On publication it reads the updated queue before closing. Errors remain beneath the progress bar; interrupted status reads offer a read-only check. The relationship queue can reopen pending/failed progress. The old work report page redirects to the SOP and its manual start API is retired. Pricing stays in the ledger for reports requested outside the app.
