...
Relationship routes share `NativeRelationshipsPanel` in the server-rendered and native shell paths. They use `DetailPageHeader`, visible `DetailFields`, the existing `RelationshipGantt`, a relationship work queue, and `DetailDangerZone` in one continuous page. Do not divide relationship details into Services/Work/History tabs. The chart's expandable left column is the services list: one named service instance per root, its current stage beneath the name, and lifecycle stages/work beneath its chevron. A single service starts expanded. Selecting an editable service name opens the shared service form above the chart. Legacy services repeat their recorded relationship lifecycle dates without cloning real work; new instances use recorded service stage events. Virtual rows cannot be dragged or opened as work items. The queue uses `List` directly below the chart; commercial history is a secondary disclosure. `PanelSectionTabs` remains available for other local sections that load on demand. Catalogue choices use `SelectorDrawer` and `SelectorOption`; stage and entry choices use `Selector`; eligible people use `AssignmentSelector` and `Assignee`.

New relationship creation asks only for the name, optional company/email/phone, and secondary Test metadata. Services are assigned from the detail. Negotiating catalogue estimates and committed sale snapshots are shown separately, grouped by currency and billing cadence. Deferred services are excluded from negotiating estimates.
