...
The service worker's recovery document (`public/offline.html`) is deliberately independent of React, authenticated documents, and Next.js chunks. It is cached and opens explicitly from the loaded workspace when connectivity is lost. Top-level documents stay on the browser's native navigation path: the service worker must not call `respondWith()` for navigation requests, because controlled standalone launches can otherwise commit an empty document in WebKit. It uses native navigation and a native chat selector rather than a record list, and a simple message timeline. Other panel destinations are disabled. Its static styling follows the same neutral surfaces and restrained status colours; it does not define alternative application primitives. Cache only a bounded recent message history, identify when it was saved, and render messages as text. Attachments require connectivity. Clear saved chats, pending sends, and staff drafts on account change or logout.
...
Client, team/direct, and portal chats share `ChatMotionViewport` for keyboard movement. The shell (or standalone portal controller) remains the sole owner of viewport geometry. Keep the chat header stationary and move the messages and composer together inside a clipped layer, using a 300ms transform with `cubic-bezier(0.32, 0.72, 0, 1)`. Resize the scrollable layout at transition boundaries and synchronously preserve its reading position; do not animate the containing iframe's height or add a second footer keyboard animation. Interrupted movement starts from its current visible position. Continuous browser viewport updates are applied directly; a revised endpoint during an existing synthetic transition uses only its remaining duration, without snapping or restarting the full animation. Repeated requests for the same pending endpoint do not restart or rewrite motion. Reduced motion skips animation. Final scroller resizing waits for an active touch scroll to settle. A missing animation finish event resolves to real layout after the duration plus 400ms, still respecting an active scroll. Page suspension/return clears interrupted gesture state and releases transient transforms.

`createComposerViewportController` shares focus and keyboard state between the workspace and portal. Valid current viewport measurements own geometry: focus, blur, suspension, resume and disposal must never manufacture a resting-height or cached-keyboard endpoint. Focus/blur only permit the first substantial movement in the expected direction to animate; small leading movement establishes continuous tracking. Accept shorter keyboards, reverse movement and keyboard dismissal without blur immediately. Repeated focus/blur notifications are idempotent. Reject zero/non-finite measurements, overscroll outside the layout, and native zoom samples. Recheck after 640ms of quiet, with at most one 80ms confirmation for metrics that arrived after their event; use the current valid measurement, never a stored target. New input, suspension, and teardown invalidate stale callbacks. Suspension retains geometry and disposal writes none. Distinguish the requested endpoint from the applied layout: after a motion owner retires, an unchanged valid measurement must repair an uncommitted taller layout. A still-active motion owner absorbs duplicate requests for its pending endpoint. Repeated taps have no cooldown. Touch/pen focus runs synchronously on a stationary pointer release; mouse focus stays on pointer-down. Do not move the composer on touch contact, cancel native activation, or use delayed blur/refocus retries.
...
If WebKit pans the locked workspace despite zero document scroll, the workspace may correct its fixed top bar, tab bar, and panel from the top bar's measured displacement on the existing viewport events. Keep the panel's bottom at the composer viewport controller's edge. Apply this correction before capturing chat motion, including focus, resume and late-metric reconciliation. The correction must leave document scroll, focused editor, selection, and drafts untouched; sample once again in the next animation frame for viewport metrics that arrive after their event, and remove the correction on teardown.

`ComposerFooter` owns the 180ms height transition for multiline drafts, reply previews, and attachment/sticker trays. Its footer stays aligned to the bottom while the message pane follows the changing height. The message observer must apply a pending height/content correction before an existing queued scroll frame records its new baseline; otherwise WebKit can consume part of a composer transition before ResizeObserver runs. Native touch/momentum continues to own scrolling. Desktop and reduced motion use immediate sizing. Keep native editor scrolling, selection, and composition intact; mark its scroller with `data-composer-scroll` so footer gesture containment permits long drafts to scroll.

For the acceptance matrix, owner boundaries and verification status, see [Communications layout contract](communications-layout-contract.md).
...
Use `Selector surface="light"` (or `SelectorTrigger surface="light"`) for client-facing light surfaces. It changes the trigger text, border and hover colours while retaining the shared drawer and keyboard behavior.
...
- Use `surface="light"` in a themed client portal; dark remains the workspace default. The light treatment uses the same layout and non-clickable summary contract.
...
- Client portals use the same light `FilterRailButton` treatment in their top-bar navigation: Results, Files, then the existing Chat action. Results contains appointments with relationship-scoped GHL reporting and a Google Ads connection placeholder in a right-hand column; on smaller screens the connection boxes stack below appointments. Use the existing `PortalSection` treatment for these task areas. GHL uses a light `Status`, a light `QuickStats` strip for open/won/lost opportunities, and plain count text for contacts and total opportunities. Only real connected metrics are displayed; connected results identify the GHL sub-account, scope, and last successful refresh. Keep the provider connection heading visible in phone landscape. Keep both content panels mounted when switching so filters, scroll positions, and uploads survive. The top bar uses a labelled `nav` with the light rail group styling, without the list-filter rail's outer spacing or divider.
...
The client portal has one vertical scroll owner beneath its sticky top bar. The greeting, active destination, complete cards, and footer move together; appointments, calendars, fulfilment, advertising reports, provider cards, and file history must not create nested page-height scrollers. Preserve a separate scroll position for each top-bar destination so switching views and returning restores that view naturally. Files remains a separate top-bar destination on every screen size, and mounted uploads continue when another destination is selected.

Chat is deliberately outside that page-scrolling contract. It remains a fixed right-side panel on desktop and a fixed fullscreen panel on mobile, with only its message pane scrolling and its composer following the visual viewport. Appointment details use the same fixed overlay shell and may scroll within their own detail body. Opening either overlay keeps the background page inert and retains its current scroll position for return.
...
- Single-line field labels and values share their first baseline. Use `DetailField multiline` for editable multiline text so the label aligns with the first line rather than the bottom of the control.
- Field icons describe the value rather than merely decorating the row: use the dedicated company, role and location marks for those relationship fields. Editable descriptions and notes use `AutoGrowTextarea`; it has no manual resize handle and expands vertically as lines are entered.
...
Connected GHL portals show a month calendar in place of the BE appointment list. The in-app Appointment Setting panel and unconnected portals retain their current behavior. A date grid is a temporal view; its selected-day appointments use the shared embedded light `List`, with title and time only. Appointment briefs and editing are deferred.

Use the automatically identified GHL owner user across booking calendars; do not expose a calendar picker. Show private blocked slots as muted Busy events and cancelled appointments with a struck-through title. Use box-less month arrows, a filled today marker and a restrained selected-day outline. Small screens show appointment counts in date cells and readable times underneath for the selected day. Keep the full month above the provider cards on smaller screens; preserve Results/Files state. State the GHL location timezone and last update. Empty cached months, unloaded months and permission failures must be distinguishable. Calendar management stays read-only.

Portal titles may use a contact name and city only through the appointment's explicit GHL contact ID. Keep the original title in a truncated supporting row and label the contact location in the selected-day list; do not imply that it is a job address. Do not infer opportunity/project details from a shared contact. Optional contact-detail loading has its own subtle text status and must not disable calendar controls.
...
Relationship routes share `NativeRelationshipsPanel` in the server-rendered and native shell paths. They use `DetailPageHeader`, visible `DetailFields`, the existing `RelationshipGantt`, a relationship work queue, and `DetailDangerZone` in one continuous page. Do not divide relationship details into Services/Work/History tabs. The chart's expandable left column shows service histories: one named service instance per root, then recorded/current lifecycle stages and real work beneath its chevron. Do not repeat the current stage beneath the service name. Omit earlier stages with no recorded visit; they were skipped rather than empty history. Generated service-group wrappers are view-only and flatten into the applicable lifecycle stage so the service name is not repeated, while genuine nested work remains nested. A single service starts expanded. Selecting a service name opens its centered detail dialog. Legacy services repeat their recorded relationship lifecycle dates without cloning real work; new instances use recorded service stage events. Virtual rows cannot be dragged or opened as work items. The queue uses `List` below the chart alongside the attached-item gallery on wide desktop; its title is the canonical open action, the operational status stays at the primary row's right edge, and service/assignment/creator metadata belongs in the secondary row. On wide desktop the complete queue region matches the height of the adjacent Services and Contact column; on smaller screens its body is capped at 24rem. In both cases, queue records scroll inside that region while its heading and pagination remain fixed. Commercial history is a secondary disclosure. `PanelSectionTabs` remains available for other local sections that load on demand. Catalogue choices use `SelectorDrawer` and `SelectorOption`; stage and entry choices use `Selector`; eligible people use `AssignmentSelector` and `Assignee`.

New relationship creation asks for the name, at least an email or phone, optional company, and secondary Test metadata. Services are assigned from the detail. Negotiating catalogue estimates and committed sale snapshots are shown separately, grouped by currency and billing cadence. Deferred services are excluded from negotiating estimates.

### Relationship card workspace and sale dialogs

Relationship detail keeps the record header, fields, History Gantt, and danger zone. `DetailPageHeader.summary` is an optional bold, wrapping row below identity and above its divider. Relationships use it for current and potential monthly recurring value; preserve separate currencies. Current value uses active service commitments; potential uses Negotiating estimates, excluding Declined.

Below History, use Queue on the left and Services/Contact on the right on wide desktop; stack Queue, Services, Contact on mobile. Attached services and contact methods are an attached-item gallery, not the navigable record `List`. Use `AttachmentCards`, `AttachmentCard`, and `AddAttachmentCard`; use `List` for catalogue choices, queue records, and commercial history. Relationship-detail galleries use the shared compact 4:3 treatment for less empty space, with larger centred thumbnails and names capped at two lines; their final plus cards use the same ratio. Other attached-item galleries remain square by default. Secondary text truncates and the detail popup exposes full content. POS cards with expanded price/assignment controls may grow below their selection face. Inactive cards are grey; verified operational failures use the restrained red border. Retain labels so colour is not the only distinction.

`CenteredDialog` is the shared compact modal for attached-item details and stepped sales. It uses the native top layer, Escape/backdrop dismissal, focus containment/restoration, an internally scrolling body, and a fixed header/footer. Use its `short` variant for compact forms that should stop at 38rem and scroll internally rather than growing to the viewport height. `AnchoredPopup` remains inside an enclosing native dialog so selector menus are visible and keyboard-accessible. Onboarding preview uses its own native top-layer dialog above the sale dialog.

The relationship fields contain name, company, role, location and notes. Contact addresses are edited inside Contact cards through the same durable background draft owner. New relationships require email or phone. No standalone messaging-confirmation block is shown. Contact facts and thumbnails load only when needed; never download message histories for a card.

The service POS opens as a centered dialog with Services, Onboarding, and Send link steps. Its first step extends the attachment cards with upfront/monthly inputs and shared assignee controls. All candidates must be reviewed before advancing. Unselected services become Declined atomically on successful sale submission; cancelling or previewing changes no service stage. Declined services remain sellable. For later is retained only as historical compatibility data, not a new choice. The second step includes the actual published onboarding preview. The final step selects currently confirmed messaging channels; direct email/calling actions remain on their contact cards.
