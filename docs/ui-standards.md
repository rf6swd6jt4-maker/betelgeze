...
- TEST client portals use the same light `FilterRailButton` treatment in their top-bar navigation: Results, Files, then the existing Chat action. Results contains appointments with GHL and Google Ads connection placeholders in a right-hand column; on smaller screens the connection boxes stack below appointments. Use the existing `PortalSection` treatment for these task areas. Keep both content panels mounted when switching so filters, scroll positions, and uploads survive; non-test portals retain their existing layout until release. The top bar uses a labelled `nav` with the light rail group styling, without the list-filter rail's outer spacing or divider.
