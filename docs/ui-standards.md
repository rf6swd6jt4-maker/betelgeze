...
Relationship routes share `NativeRelationshipsPanel` in the server-rendered and native shell paths. They use `DetailPageHeader`, `DetailFields`, `DetailDangerZone`, and `List` for services, work and commercial history. `PanelSectionTabs` shares `PanelTabs` geometry for local sections whose content loads on demand; it does not prefetch hidden section data. Catalogue choices use `SelectorDrawer` and `SelectorOption`; stage and entry choices use `Selector`; eligible people use `AssignmentSelector` and `Assignee`.
