...
`CenteredDialog` is the shared compact modal for attached-item details and stepped sales. It uses the native top layer, Escape/backdrop dismissal, focus containment/restoration, an internally scrolling body, and a fixed header/footer. `AnchoredPopup` remains inside an enclosing native dialog so selector menus are visible and keyboard-accessible. Onboarding preview uses its own native top-layer dialog above the sale dialog.
