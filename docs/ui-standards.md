...
Below History, use Queue on the left and Services/Contact on the right on wide desktop; stack Queue, Services, Contact on mobile. Attached services and contact methods are an attached-item gallery, not the navigable record `List`. Use `AttachmentCards`, `AttachmentCard`, and `AddAttachmentCard`; use `List` for catalogue choices, queue records, and commercial history. Cards have fixed thumbnails, wrapping names, muted secondary text, and a final plus card. Inactive cards are grey; verified operational failures use the restrained red border. Retain labels so colour is not the only distinction.
