...
Mobile staff Communications opens as a resident same-document panel (`NativeCommunicationsTab`). Its renderer remains stable through orientation/breakpoint changes so drafts, selection and loaded data are not discarded. Desktop-opened tabs and the client portal retain their existing renderer. Reuse the workspace navigation, account-scoped record cache, authenticated GETs and tab identity; hidden resident panels must not write the active URL, acquire focus, display floating menus or play gallery media.

`observeMobileWorkspaceViewport` is the sole mobile resident geometry owner. In layout-viewport coordinates the visual viewport supplies `offsetTop` (origin) and `height` (usable size). Header top is the origin; tabs follow the measured header; the panel follows both chrome rows and has height `visual height - header height - tabs height`. Never add the origin to the available height or independently accumulate a measured header correction. Reject invalid/zoom samples and retain the last usable layout. Focus/blur is intent, not a keyboard measurement. Apply valid current bounds directly; one event-triggered next-frame read accommodates metrics delivered after the event, without a polling or animation loop.

Inside this resident surface, `ChatMotionViewport` is plain flex layout and `ComposerFooter` takes its natural content height. No synthetic whole-chat transform, larger temporary layout, separate footer height animation or delayed final resize is permitted. The message observer remains the sole scroll owner: preserve the latest-message gap, retain history anchors relative to the pane bottom when its height changes, and yield scroll writes during a touch drag or momentum. Editor growth, reply and attachment trays participate in the same layout. Native selection, composition and internal draft scrolling remain available. A focused editor retains keyboard focus when its send button is tapped.

The legacy iframe/portal path continues to use `createComposerViewportController`, `ChatMotionViewport` and `ComposerFooter` with its prior timing/gesture contract. It is suspended whenever the mobile resident owner is active. Do not mix either owner's geometry or corrective transforms into the other. Legacy document-origin recovery remains guarded at rest; the mobile owner never writes document scroll positions.

A departing mode/tab cancels pending focus and message-hold gestures and hides its portal overlays. This does not discard its draft or alter the existing reading/notification policy. Read eligibility continues to require the exact active native tab, foreground attention and an actually visible, unobstructed newest saved message.

For acceptance cases, evidence limits and rollback, see [Communications layout contract](communications-layout-contract.md). Synthetic Chromium/WebKit tests are not physical iPhone/Android keyboard evidence.
