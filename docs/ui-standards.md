...
Native team/direct chats also offer `Quote` for saved messages with text. It enters a selection step in the original body, with `MessageQuoteSelection` replacing the composer until the user confirms or cancels. During selection, native text selection and its touch handles own the gesture; bubble holds, swipes, link activation, and checklist changes must not compete. Arrow keys move the selection caret, Shift extends it, and Escape cancels. Client chats and the client portal do not offer quoting.
...
Composer touch containment must yield to a non-collapsed selection in the focused editor. Check both touch start and movement (long press can select after contact), and preserve that gesture until release even if the selection handles cross. Ordinary unselected drags at editor scroll boundaries still prevent page panning.
