...
Relationship routes share `NativeRelationshipsPanel` in the server-rendered and native shell paths. They use `DetailPageHeader`, visible `DetailFields`, the existing `RelationshipGantt`, a relationship work queue, and `DetailDangerZone` in one continuous page. Do not divide relationship details into Services/Work/History tabs. The chart's expandable left column shows service histories: one named service instance per root, its current stage beneath the name, and lifecycle stages/work beneath its chevron. A single service starts expanded. Selecting a service name opens its centered detail dialog. Legacy services repeat their recorded relationship lifecycle dates without cloning real work; new instances use recorded service stage events. Virtual rows cannot be dragged or opened as work items. The queue uses `List` below the chart alongside the attached-item gallery on wide desktop; commercial history is a secondary disclosure. `PanelSectionTabs` remains available for other local sections that load on demand. Catalogue choices use `SelectorDrawer` and `SelectorOption`; stage and entry choices use `Selector`; eligible people use `AssignmentSelector` and `Assignee`.
...
Below History, use Queue on the left and Services/Contact on the right on wide desktop; stack Queue, Services, Contact on mobile. Attached services and contact methods are an attached-item gallery, not the navigable record `List`. Use `AttachmentCards`, `AttachmentCard`, and `AddAttachmentCard`; use `List` for catalogue choices, queue records, and commercial history. Gallery cards and their final plus cards share a square aspect ratio on every viewport. Use compact thumbnails on mobile, names capped at two lines, and truncated secondary text; the detail popup exposes full content. POS cards with expanded price/assignment controls may grow below their selection face. Inactive cards are grey; verified operational failures use the restrained red border. Retain labels so colour is not the only distinction.
