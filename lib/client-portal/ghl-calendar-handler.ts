...
function project(value:Record<string,unknown>){
 ...
 const snapshot=raw?.source==="owner-user"?{source:raw.source,owner:{id:raw.owner.id,name:raw.owner.name},timezone:raw.timezone,month:raw.month,events:raw.events.map(e=>({id:e.id,title:e.title,start:e.start,end:e.end,status:e.status,kind:e.kind,allDay:e.allDay}))}:null
