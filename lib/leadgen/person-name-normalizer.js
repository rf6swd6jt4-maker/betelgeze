import { givenNameCount, nameFrequencyCounts, nameFrequencySource, surnameCount } from "./name-frequency-lookup.js"
...
const NAME_FREQUENCY_TOTALS = nameFrequencySource()
const GIVEN_NAME_RATE_FLOOR = 0.25 / NAME_FREQUENCY_TOTALS.totalGivenObservations
const SURNAME_RATE_FLOOR = 0.25 / NAME_FREQUENCY_TOTALS.totalSurnameObservations
...
function nameRates(key) {
    const counts = nameFrequencyCounts(key)
    return {
        given: counts.given / NAME_FREQUENCY_TOTALS.totalGivenObservations,
        surname: counts.surname / NAME_FREQUENCY_TOTALS.totalSurnameObservations,
        givenCount: counts.given,
        surnameCount: counts.surname,
    }
}

function logOdds(numerator, denominator) {
    return Math.log((numerator + GIVEN_NAME_RATE_FLOOR) / (denominator + SURNAME_RATE_FLOOR))
}

function givenNameEvidence(key) {
    const rates = nameRates(key)
    const evidence = logOdds(rates.given, rates.surname)
    if (rates.givenCount > 0) return evidence
    if (COMMON_GIVEN_NAMES.has(key)) return Math.max(evidence, 0.9)
    return evidence
}

function surnameEvidence(key) {
    const rates = nameRates(key)
    return -logOdds(rates.given, rates.surname)
}
...
function givenNameWeight(key) {
    ...
    const count = givenNameCount(key)
    if (count > 0) return 42 + Math.log10(count) * 11
    if (COMMON_GIVEN_NAMES.has(key)) return 62
    if (surnameCount(key) > 0) return 6
    return 18
}
...
function surnameWeight(key) {
    ...
    const count = surnameCount(key)
    if (count > 0) return 42 + Math.log10(count) * 11
    if (givenNameCount(key) > 0 || COMMON_GIVEN_NAMES.has(key)) return 7
    return 24
}
...
function givenSequenceLikelihood(keys) {
    ...
    const evidence = keys.reduce((total, key) => total + givenNameEvidence(key), 0)
...
function surnameSequenceLikelihood(keys) {
    ...
    const evidence = substantiveKeys.reduce((total, key) => total + surnameEvidence(key), 0)
    return { score, evidence }
}
...
function candidateScore(tokens, sourceText, options) {
    ...
    let score = 42
    if (first && (givenNameCount(first) > 0 || COMMON_GIVEN_NAMES.has(first))) score += 26
...
export function maybeNormaliseLastNameFirstPersonName(value, options = {}) {
    ...
    const requiredEvidence = sourceHintsLastFirst ? 1.8 : 4.5
