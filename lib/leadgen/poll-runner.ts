...
function executablePlan(sourcePlan: LeadgenSourcePlanItem[]) {
    return {
        osmPlan: sourcePlan.find((source) => source.key === "osm"),
        ...
        preSeedPipelinePlans: sourcePlan.filter((source) => ["overture", "sam_gov"].includes(source.key) && executableLeadgenSources.has(source.key) && source.industries.length > 0 && source.locations.length > 0),
...
export async function createInitialLeadgenPollTasks({ workspaceId, pollId, sourcePlan }: { workspaceId: string; pollId: string; sourcePlan: LeadgenSourcePlanItem[] }) {
    const { osmPlan, stateLicensingPlan, preSeedPipelinePlans } = executablePlan(sourcePlan)
    ...
    const initialTaskCounts = await Promise.all([
        preSeedPipelinePlans.length ? createPipelineTasksForPoll({ workspaceId, pollId, plans: preSeedPipelinePlans }) : 0,
        runnableOsmPlan ? createOsmTasksForPoll({ workspaceId, pollId, plan: runnableOsmPlan }) : 0,
        runnableStateLicensingPlan ? createStateLicensingTasksForPoll({ workspaceId, pollId, plan: runnableStateLicensingPlan }) : 0,
    ])
    return initialTaskCounts.reduce((total, count) => total + count, 0)
}
...
export async function processLeadgenPoll({ workspaceId, pollId }: { workspaceId: string; pollId: string }) {
    ...
    const { osmPlan, stateLicensingPlan, websitePlan, preSeedPipelinePlans, postSeedPipelinePlans } = executablePlan(sourcePlan)
    const runnableOsmPlan = osmPlan && osmPlan.industries.length > 0 && osmPlan.locations.length > 0 ? osmPlan : null
    const runnableStateLicensingPlan = stateLicensingPlan && stateLicensingPlan.industries.length > 0 && stateLicensingPlan.locations.length > 0 ? stateLicensingPlan : null
    ...
    if (runnableOsmPlan) await processOsmPoll(pollId, workspaceId, { finalize: false })
    if (runnableStateLicensingPlan) await processStateLicensingPoll(pollId, workspaceId, { finalize: false })
