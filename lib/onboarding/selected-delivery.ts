...
export function selectedOnboardingDestinations(payload: Record<string, unknown> | null, current: MessagingChoice[], allowWhatsAppTemplate = false) {
    ...
    return choices.map((choice, index) => {
        if (!current.some(candidate => candidate.provider === choice.provider && candidate.address === choice.address &&
            (candidate.state === "active" && candidate.canSend || choice.provider === "meta_whatsapp" && allowWhatsAppTemplate && candidate.enabled && candidate.state !== "broken"))) {
            throw new Error("A chosen contact method changed or is unavailable. Restore its configured connection before retrying this link.")
        }
