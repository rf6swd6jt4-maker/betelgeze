...
export const RELATIONSHIP_BACKGROUND_FIELDS = ["industryValue", "websiteUrl", "locationValue", "primaryPersonName", "businessName", "primaryContactRole", "primaryPhone", "whatsappPhone", "communicationPrimaryProvider", "communicationDeliveryMode", "primaryEmail", "description"] as const
const UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i

export function relationshipBackgroundValues(value: RelationshipBackgroundValues): RelationshipBackgroundValues {
    return Object.fromEntries(RELATIONSHIP_BACKGROUND_FIELDS.filter(key => !["locationValue", "industryValue", "websiteUrl"].includes(key) || value[key] !== undefined).map((key) => [key, value[key]])) as RelationshipBackgroundValues
}
export function parseRelationshipBackgroundCommand(value: unknown): RelationshipBackgroundCommand | null {
    ...
    if (Object.keys(values).length !== RELATIONSHIP_BACKGROUND_FIELDS.length - ["locationValue", "industryValue", "websiteUrl"].filter(key => values[key] === undefined).length || RELATIONSHIP_BACKGROUND_FIELDS.some((key) => (!["locationValue", "industryValue", "websiteUrl"].includes(key) || values[key] !== undefined) && (typeof values[key] !== "string" || (values[key] as string).length > (key === "description" ? 20_000 : 2_000)))) return null
