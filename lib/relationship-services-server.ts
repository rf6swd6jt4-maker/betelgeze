...
export function relationshipBackgroundDraft(r: RelationshipRecord): RelationshipDraft {
    return {
        industryValue: r.industry_value ?? "", websiteUrl: r.website_url ?? "", locationValue: r.location_value ?? "", primaryPersonName: r.primary_person_name, businessName: r.business_name ?? "", primaryContactRole: r.primary_contact_role ?? "",
