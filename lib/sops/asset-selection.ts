...
export function validateAssetSelections(plan:SopWorkPlan,source:SopInterpretation,candidates:AssetCandidate[],required=false){
 ...
 for(const task of plan.tasks){
  ...
  for(const a of choices){const candidate=map.get(a.asset_id),step=source.steps[a.source_step-1]
   ...
   for(const [quote,context] of [[a.source_quote,step.instruction],[a.asset_quote,candidate.description]])if(typeof quote!=='string'||quote.trim().length<12||quote.length>400||!normalized(context).includes(normalized(quote)))throw new Error('The work plan contains unsupported attachment evidence.')
