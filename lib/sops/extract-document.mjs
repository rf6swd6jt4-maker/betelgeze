...
const text = value => String(value??'').replace(/[\p{Cc}\p{Cs}]/gu,' ').replace(/\s+/g,' ').trim()
async function retain(canvas, location, context, method, ordinal, attachable=true) {
  ...
  images.push({ordinal,attachable,location:Array.from(text(location)).slice(0,300).join(''),context:Array.from(text(context)).slice(0,2000).join(''),method,width:canvas.width,height:canvas.height,hash:createHash('sha256').update(bytes).digest('hex'),data:bytes.toString('base64'),thumbnail:(await thumbnail.encode('webp',75)).toString('base64')})
}
