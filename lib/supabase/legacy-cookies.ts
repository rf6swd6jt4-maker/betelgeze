...
import { SESSION_COOKIE_NAME } from "@/lib/supabase/session-cookies"
...
export function clearLegacyHostOnlyAuthCookies(request: NextRequest, response: NextResponse) {
    const names = new Set(request.cookies.getAll().map((cookie) => cookie.name).filter((name) => /^sb-[a-z0-9-]+-auth-token(?:\.\d+)?$/i.test(name) || name === SESSION_COOKIE_NAME || name.startsWith(`${SESSION_COOKIE_NAME}.`)))
