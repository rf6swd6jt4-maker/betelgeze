...
export function workspaceLaunchUrlForRestore(value: string, workspaceSlug: string) {
    ...
    if (segments[0] === "appointment-setting") return `${prefix}client-connections`
    if (segments.length === 2 && ["relationships", "onboarding", "work", "work-items", "assets", "notes"].includes(segments[0])) {
