...
export async function proxy(request: NextRequest) {
    ...
    if (domain === LEADGEN_HOST) {
        ...
        const workspacePath = path.match(/^\/([a-z0-9][a-z0-9-]*)(?:\/(settings))?\/?$/i)
        if (workspacePath) {
            const headers = new Headers(request.headers)
            headers.set("x-betelgeze-workspace-slug", workspacePath[1].toLowerCase())
            return withRewrite(request, `/leadgen/${workspacePath[1].toLowerCase()}${workspacePath[2] ? `/${workspacePath[2]}` : ""}`, headers)
        }
    }
