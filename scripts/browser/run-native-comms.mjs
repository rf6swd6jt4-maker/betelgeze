...
if (selected.some(value => !["chromium", "webkit"].includes(value))) throw Error("Usage: run-native-comms.mjs [chromium|webkit]")
...
try {
    ...
    await writeFile("browser-results/native-comms.json", JSON.stringify({ observedAt: new Date().toISOString(), reports, limits: "Loopback synthetic component/helper checks. Not production latency, authenticated UI or physical-device evidence." }, null, 2))
}
