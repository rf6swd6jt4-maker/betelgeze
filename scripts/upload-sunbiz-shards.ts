...
function usage() {
    return [
        "Usage:",
        "  npm run upload:sunbiz-shards -- --dir .sunbiz-shards --prefix sunbiz/fl",
        "",
        "Options:",
...
function parseArgs(argv: string[]): CliOptions {
    const options: CliOptions = {
        dir: ".sunbiz-shards",
        prefix: process.env.SUNBIZ_R2_PREFIX ?? "sunbiz/fl",
        dryRun: false,
        concurrency: 8,
    }
...
async function uploadShards(options: CliOptions) {
    ...
    console.log(`Uploading ${files.length.toLocaleString()} shard file${files.length === 1 ? "" : "s"} (${formatBytes(totalBytes)}) to R2 prefix ${options.prefix}...`)
    if (options.dryRun) {
