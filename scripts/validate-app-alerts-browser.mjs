...
await mkdir(output,{recursive:true});await mkdir(join(fixture,'app','reader'),{recursive:true});await mkdir(join(fixture,'app','native'),{recursive:true})
for(const [from,to] of [['host.tsx.fixture','app/page.tsx'],['native-host.tsx.fixture','app/native/page.tsx'],['reader.tsx.fixture','app/reader/page.tsx'],['layout.tsx.fixture','app/layout.tsx']])await writeFile(join(fixture,to),await readFile(join(root,'tests','fixtures','app-alerts',from)))
...
const results=[];for(const [engine,type,executablePath] of [['chromium',chromium,process.env.CHROMIUM_EXECUTABLE],['webkit',webkit,process.env.WEBKIT_EXECUTABLE]]){
 ...
 try{for(const viewport of [{width:1280,height:900},{width:390,height:844}]){for(const hostMode of ['iframe','native']){
 ...
 // A hidden resident frame/native panel must neither save reads nor advertise reading.
 await page.locator('#switch').click();const hiddenReads=reads.length;await reader().evaluate(()=>window.addMessage());await page.waitForTimeout(300);assert.equal(reads.length,hiddenReads);assert.equal(activities.at(-1).active,false);
 await page.locator('#switch').click();await zero();
 // A shell viewer covering either host must block reading until dismissed.
 await page.locator('#cover').click();const coveredReads=reads.length;await reader().evaluate(()=>window.addMessage());await page.waitForTimeout(300);assert.equal(reads.length,coveredReads);assert.equal(activities.at(-1).active,false);
 ...
 fail=false;await reader().evaluate(()=>window.dispatchEvent(new Event('online')));await zero();
 assert.deepEqual(errors,[]);results.push({engine,viewport,hostMode,passed:true,readRequests:reads.length,activityRequests:activities.length});console.log(JSON.stringify(results.at(-1)));await context.close();
