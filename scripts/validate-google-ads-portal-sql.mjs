...
try {
 ...
 await db.exec(await readFile(`${repositoryRoot}/supabase/migrations/20260915090000_google_ads_service_reporting_split.sql`,'utf8'))
 await db.exec(await readFile(`${repositoryRoot}/supabase/migrations/20260918150000_google_ads_account_replacement.sql`,'utf8'))
 ...
 const report=(action='read',attempt=null,config=null,data=null,t=token,kind='search')=>one('select client_portal_google_ads_report($1,$2,$3,$4,$5,$6,$7,$8,$9) v',[t,w,'last30',kind,action,attempt,config,data,null])
 ...
 await assert.rejects(report('finish',a,'encrypted-fixture',{kind:'search',customerId:'9999999999'}),/Invalid account/)
 const saved=await report('finish',a,'encrypted-fixture',{kind:'search',customerId:customer,spend:5});assert.equal(saved.snapshot.report.spend,5)
 const localStarted=await report('begin',b,null,null,token,'local_services');assert.equal(localStarted.customerId,customer)
 await assert.rejects(report('finish',b,'encrypted-fixture',{kind:'search',customerId:customer},token,'local_services'),/Invalid account/)
 const localSaved=await report('finish',b,'encrypted-fixture',{kind:'local_services',customerId:customer,leads:2},token,'local_services');assert.equal(localSaved.snapshot.report.leads,2)
 assert.equal((await query('select count(*)::int n from relationship_google_ads_reports'))[0].n,2)
 ...
 assert.equal((await report()).snapshot,null)
 await assert.rejects(report('finish',b,'encrypted-fixture',{kind:'search',customerId:customer}),/changed/)
 ...
 await db.exec("update relationship_google_ads_connections set updated_at=now()-interval '10 seconds';")
 const replacement=await begin(token,'3456789012',a)
 assert.equal(replacement.previousCustomerId,customer)
 assert.equal((await query('select status from relationship_google_ads_connections'))[0].status,'needs_attention')
 ...
 assert.equal((await finish(a,'encrypted-fixture','pending')).status,'pending')
