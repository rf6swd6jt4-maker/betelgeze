...
try {
    ...
    const old = await measure("select count(*)::int n from communication_native_messages_bounded($1,null,4000)",[w])
    const inbox = await measure("select communication_native_inbox($1) inbox",[w])
    const recent = await measure("select count(*)::int n from communication_native_messages_bounded($1,$2,60)",[w,c])
    const value = inbox.rows[0].inbox
    assert.equal(value.messages.length,1)
    assert.equal(value.unread[0].messages.length,6000)
