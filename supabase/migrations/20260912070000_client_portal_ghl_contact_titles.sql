...
-- Independent title leases never block a calendar/month refresh.
alter table client_portal_secure.ghl_calendar_snapshots
 add column if not exists names_operation_id uuid,
 add column if not exists names_lease_until timestamptz,
 add column if not exists names_attempted_at timestamptz,
 add column if not exists names_base_refreshed_at timestamptz;

create or replace function public.client_portal_ghl_calendar_names(p_session_token text,p_workspace_id uuid,p_action text,p_refreshed_at timestamptz,p_operation_id uuid default null,p_labels jsonb default null)
returns jsonb language plpgsql security definer set search_path='' as $$
declare
 access jsonb;
 target uuid;
...
begin
 ...
 if access ? 'failure' then return access; end if;
 if not coalesce((access->>'connected')::boolean,false) then return jsonb_build_object('failure','credentials_missing'); end if;
 if p_action not in ('begin','finish','fail') or p_refreshed_at is null or p_operation_id is null then return jsonb_build_object('failure','response'); end if;
 select relationship_id into target from public.client_portal_sessions where session_token=p_session_token and workspace_id=p_workspace_id;
 perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('portal-ghl:'||target::text,0));
 select * into connection from client_portal_secure.ghl_connections where relationship_id=target and workspace_id=p_workspace_id;
 select * into saved from client_portal_secure.ghl_calendar_snapshots where relationship_id=target;
 if connection.vault_secret_id is null then return jsonb_build_object('failure','credentials_missing'); end if;
 if saved.refreshed_at is distinct from p_refreshed_at or saved.revision is distinct from connection.calendar_revision or saved.snapshot->>'source' is distinct from 'owner-user' then return jsonb_build_object('failure','changed'); end if;
 if p_action='begin' then
  ...
  if saved.names_base_refreshed_at=p_refreshed_at then
  ...
  update client_portal_secure.ghl_calendar_snapshots set names_operation_id=p_operation_id,names_lease_until=now()+interval '30 seconds',names_attempted_at=now(),names_base_refreshed_at=p_refreshed_at where relationship_id=target;
  select decrypted_secret into secret_value from vault.decrypted_secrets where id=connection.vault_secret_id;
  return jsonb_build_object('locationId',connection.location_id,'privateToken',secret_value,'snapshot',saved.snapshot);
 end if;
 if saved.names_operation_id is distinct from p_operation_id or saved.names_base_refreshed_at is distinct from p_refreshed_at or saved.names_lease_until<=now() then return jsonb_build_object('failure','changed'); end if;
 if p_action='finish' then
...
$$;
revoke all on function public.client_portal_ghl_calendar_names(text,uuid,text,timestamptz,uuid,jsonb) from public,anon,authenticated;
grant execute on function public.client_portal_ghl_calendar_names(text,uuid,text,timestamptz,uuid,jsonb) to service_role;
