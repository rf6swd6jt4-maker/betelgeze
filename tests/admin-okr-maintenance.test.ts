...
test("the private activity console covers core automation producers", async () => {
    ...
    assert.match(activityPage, /ActivityTrendsRemote/)
    const activityLoader = await readFile("lib/workspace-native-admin.ts", "utf8")
    assert.match(activityLoader, /buildAdminActivityMetricBundle/)
