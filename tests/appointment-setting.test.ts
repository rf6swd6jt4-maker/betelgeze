...
test("Appointment Setting appointments are relationship and service scoped with secure realtime reads", () => {
    ...
    assert.match(actions, /saveAppointmentSettingDraft/)
})
...
test("Appointment Setting onboarding config drives table fields and remote links", () => {
    ...
    const editor = readFileSync("components/appointment-setting/AppointmentDraftEditor.tsx", "utf8")
    assert.match(editor, /field\("appointment_date", "Date", "date", true\)/)
    assert.match(editor, /field\("appointment_time", "Time", "time", true\)/)
...
test("Appointment Setting drafts submit once and create an automated Communications message", () => {
    ...
    assert.match(readFileSync("components/appointment-setting/AppointmentDraftEditor.tsx", "utf8"), /"Submitting…" : "Submit appointment"/)
