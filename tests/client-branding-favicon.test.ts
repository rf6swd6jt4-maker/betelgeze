...
test("public asset endpoints are same-origin, immutable, and keep the staff manifest unchanged", () => {
    ...
    assert.match(logoRoute, /status: 307/u)
    assert.match(logoRoute, /Location: requestUrl\.toString\(\)/u)
    assert.match(logoRoute, /"Cache-Control": "private, no-store"/u)
    assert.doesNotMatch(logoRoute, /Response\.redirect/u)
