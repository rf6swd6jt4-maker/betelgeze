...
export async function docxFixture(count=2,external=false,complex=false){
 ...
 await writer.add('word/document.xml',new TextReader(`<w:document xmlns:w="word" xmlns:wp="drawing" xmlns:a="drawingml" xmlns:pic="picture" xmlns:r="relationships"><w:body><w:p><w:pPr><w:pStyle w:val="Heading1"/></w:pPr><w:r><w:t>Configure conversion tracking</w:t></w:r></w:p>${Array.from({length:count},(_,i)=>`<w:p><w:r><w:t>Verify tracking setting ${i+1} using this screenshot.</w:t>${image}</w:r></w:p>`).join('')}</w:body></w:document>`))
